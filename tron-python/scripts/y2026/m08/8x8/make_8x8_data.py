...
def main():
    ...
    for i in range(1000):
        ...
        oracle_table, _special_cases = make_oracle_data(game)

        print(f"Game #{i}, {len(oracle_table)} examples created")

        tactics = make_tactics_from_oracle(oracle_table, _special_cases, 5)
        ...
        oracle_examples = list(oracle_table.values())
    ...
    with open(out_dir / f"{grid_dim}x{grid_dim}_validation_{i+1}games.bin", "wb") as f:
