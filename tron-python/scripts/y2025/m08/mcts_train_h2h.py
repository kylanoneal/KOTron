...
def main():
    ...
    state_dict = torch.load(
        r"my_repos/KOTron/tron-python/scripts/y2025/m08/runs/20250801-232657_mcts_v6_server/checkpoints/mcts_v6_server_4590.pth" \
    )
    ...
    for train_iter in range(n_train_sim_cycles):
        ...
        for _ in tqdm(range(n_games_per_loop)):
            ...
            while game_status.status == GameStatus.IN_PROGRESS:
                ...
                root: MCTS.Node = MCTS.search(model, game, 0, n_iterations=mcts_iters, root=None)

                p1_dir, p2_dir, _ = MCTS.get_move_pair(root, 0, temp=0.01)
        ...
        dataloader = make_dataloader(
            all_game_states, batch_size=batch_size
        )
