...
#[derive(Clone)]
pub struct GameState2D {
    // These should all be private?
    pub grid: Vector<Vector<bool>>,
    pub players: Vector<Player>,
}

/// Creates a new `GameState2D` with the given players
pub fn new_game(players: Vector<Player>, num_rows: usize, num_cols: usize) -> GameState2D {
    ...
    GameState2D { players, grid }
}

// Moves players in their direction, also does collision checking
pub fn next(game: &GameState2D, directions: &[Direction]) -> GameState2D {
...
pub fn get_possible_directions(game: &GameState2D, player_index: usize) -> Vec<Direction> {
...
pub fn get_status(game_state: &GameState2D) -> StatusInfo {
...
pub fn print_grid(game_state: &GameState2D) {
