...
if __name__ == "__main__":
    ...
    state_dict = torch.load(
        "C://Users//kylan//Documents//code//repos//KOTron//python//tasks//2024_12_7_optimize//runs//20241207-124612_fastnet_train_v1//checkpoints//fastnet_train_v1_20.pth"
    )

    torch_model = FastNet(grid_dim=10)
    torch_model.load_state_dict(state_dict)
    torch_model = torch_model.to(device)

    fast_model_random = StandardTronModel(torch_model, device)

    state_dict = torch.load(
        "C://Users//kylan//Documents//code//repos//KOTron//python//tasks//2024_12_7_optimize//runs//20241207-174222_fastnet_self_train_v2//checkpoints//fastnet_self_train_v2_20.pth"
    )
    torch_model = FastNet(grid_dim=10)
    torch_model.load_state_dict(state_dict)
    torch_model = torch_model.to(device)

    fast_model_self_train = StandardTronModel(torch_model, device)
        ...
        for j in tqdm(range(n_games_per_loop)):
            ...
            while game.status == GameStatus.IN_PROGRESS:
                ...
                p1_direction_update = choose_direction_minimax(
                    fast_model_random, game, player_index=0, opponent_index=1, depth=2
                )

                p2_direction_update = choose_direction_minimax(
                    fast_model_self_train, game, player_index=1, opponent_index=0, depth=2
                )
